      ...
      <div className="animate-reveal">
        <div className="relative h-72 bg-s1">
          ...
          <div className="absolute top-8 left-6">
            <button onClick={() => navigate(-1)} className="w-12 h-12 glass rounded-full flex items-center justify-center text-white active-scale">←</button>
          </div>
        </div>
        ...
        <div className="px-6 -mt-20 relative z-10">
          ...
          <div className="sticky top-4 z-40 mb-10 overflow-x-auto no-scrollbar flex gap-2">
              ...
              <button key={m.cat} onClick={() => setCurCat(m.cat)}
                  className={`flex-shrink-0 px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${curCat === m.cat ? 'bg-brand-orange text-white' : 'glass text-t2'}`}>
                  {m.cat}
              </button>
              ))}
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 mb-24">
              {currentCategory?.items.map((item, idx) => (
                  <div key={item.id} className="glass p-4 rounded-[28px] flex gap-5 active-scale border-white/5"
