...
// OPTIMIZATION: Added loading="lazy" and decoding="async" to prevent image rendering from blocking main thread
            ...
            <div key={catIdx}>
              ...
              <div className="grid grid-cols-1 gap-4">
                {cat.items.map((item, itemIdx) => (
                  <div key={item.id} className="glass p-4 rounded-[28px] flex gap-4 items-center border-white/5 transform-gpu">
                    <div className="w-16 h-16 rounded-2xl overflow-hidden shrink-0"><ImageWithFallback src={item.img} className="w-full h-full object-cover bg-s2" alt={item.n} /></div>
  ...
  if (user.role === 'courier') {
      return (
        <div className="px-6 py-6 animate-reveal">
            ...
            <div className="relative mb-10 overflow-hidden rounded-[32px] shadow-xl h-44 mx-1 transform-gpu">
                <div 
                  ref={carouselRef} 
                ...
                {slides.map((_, i) => (
                    <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${heroSlideIndex === i ? 'w-5 bg-white' : 'w-1.5 bg-white/40'}`}></div>
                ))}
                </div>
            </div>
            ...
            <button 
                onClick={toggleShift}
                className={`w-full py-6 rounded-[24px] font-geologica font-black text-sm uppercase tracking-[3px] active-scale transition-all duration-300 mb-8 border border-white/5 ${user.isOnline ? 'bg-gradient-to-r from-green-600 to-emerald-500 text-white shadow-lg' : 'glass text-t2'}`}
            >
                {user.isOnline ? 'Завершити зміну' : 'Розпочати зміну'}
            </button>
            
            {!user.isOnline && (
                <div className="text-center p-8 opacity-40 transform-gpu">
            ...
            {user.isOnline && (
                 <div className="glass p-6 rounded-[28px] text-center border-brand-orange/20 relative overflow-hidden transform-gpu">
          ...
          {slides.map((slide, i) => (
            <div key={i} className="min-w-full snap-start relative h-full">
              ...
              <div className="relative p-6 flex flex-col justify-center h-full">
                  ...
                  <div className="text-[9px] font-black tracking-[3px] opacity-70 uppercase mb-1 text-white">{slide.sub}</div>
                  <h2 className="font-geologica text-xl font-black leading-[1.1] mb-4 text-white whitespace-pre-line">{slide.title}</h2>
                  <button onClick={() => navigate('/rests')} className="self-start bg-white text-black rounded-full px-6 py-2.5 font-geologica text-[10px] font-black uppercase shadow-md active-scale">Замовити</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-5 overflow-x-auto overflow-y-hidden no-scrollbar -mx-5 px-10 select-none pb-4">
          {restaurants.slice(0, 4).map((r, i) => (
            <div key={r.id} className="flex-shrink-0 w-64 cursor-pointer" onClick={() => navigate('/rest', { state: { restaurant: r } })}>
              <div className="relative h-40 rounded-3xl overflow-hidden mb-3 shadow-lg active-scale transform-gpu">
                  <ImageWithFallback src={r.img} alt={r.n} className="w-full h-full object-cover bg-s2" />
