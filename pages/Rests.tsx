    ...
    <div className="px-6 py-6 animate-reveal">
      ...
      <div className="flex gap-2 mb-10 overflow-x-auto no-scrollbar -mx-2 px-2">
        {filters.map(f => (
          <button 
            key={f}
            onClick={() => setFilter(f)}
            className={`flex-shrink-0 px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${filter === f ? 'bg-brand-orange text-white shadow-lg shadow-brand-orange/20' : 'glass text-t2 border-transparent'}`}
          >
            {f}
          </button>
        ))}
      ...
      <div className="space-y-8 pb-10">
        {filtered.map((r, i) => (
          ...
          >
            <div className="relative h-56 rounded-[32px] overflow-hidden mb-4 shadow-lg active-scale border border-white/5 transform-gpu">
                <ImageWithFallback src={r.img} className="w-full h-full object-cover transition-transform duration-500 bg-s2" alt={r.n} />
