...
const Navigation: React.FC<NavigationProps> = ({ cartCount, role }) => {
  ...
  const indicatorWidthPercent = isAdmin ? 35 : 30; // Relative to the single tab width
  const indicatorOpacity = activeIndex === -1 ? 0 : 1;

  return (
    <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 ${containerWidth} h-[76px] glass rounded-[32px] z-50 flex items-center justify-around shadow-[0_20px_40px_rgba(0,0,0,0.5)] border-none bottom-nav`}>
      
      {/* OPTIMIZED GPU ANIMATION WRAPPER */}
      <div 
        className="absolute top-0 left-0 h-full nav-indicator-wrapper flex justify-center"
        style={{ 
      ...
      >
        <div 
            className="absolute top-0 h-[3.5px] bg-brand-orange rounded-full shadow-[0_0_12px_rgba(255,92,0,0.8)]"
